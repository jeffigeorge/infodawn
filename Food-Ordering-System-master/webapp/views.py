...
from .forms import CustomerSignUpForm, RestuarantSignUpForm, CustomerForm, RestuarantForm, Productform
from django.contrib.auth.decorators import login_required
from collections import Counter
from django.urls import reverse
from django.db.models import Q
from .models import Customer, Restaurant, Item, Menu, Order, orderItem, User, Product
    ...
    if request.method == "POST":
        ...
        if user is not None:
            if user.is_active:
                ...
                return render(request, 'webapp/adminlogin.html', {'error_message': 'Your account disable'})
        else:
            return render(request, 'webapp/adminlogin.html', {'error_message': 'Invalid Login'})
    return render(request, 'webapp/adminlogin.html')
...
def AdminProfile(request, pk=None):
    ...
    return render(request, 'webapp/adminprofile.html', {'user': user})


class Usersview(ListView):
    template_name = 'webapp/adminusers.html'
    model = Customer
    context_object_name = 'list'


class Restview(ListView):
    template_name = 'webapp/adminrestview.html'
    model = Restaurant
    context_object_name = 'list'


class Addpdtview(CreateView):
    template_name = 'webapp/adminaddgrocery.html'
    model = Product
    form_class = Productform
    success_url = '/profile/admin/'


class Viewpdtview(ListView):
    template_name = 'webapp/adminviewgrocery.html'
    model = Product
    context_object_name = 'list'


class Uppdtview(UpdateView):
    template_name = 'webapp/adminaddgrocery.html'
    model = Product
    fields = ['Product_name', 'Manufacturing_date', 'Net_Quantity', 'Price', 'Description', 'Image', 'Test_Status',
              'Availability']
    success_url = '/profile/viewpdt/'


class Delpdtview(DeleteView):
    template_name = 'webapp/delbtn.html'
    model = Product
    success_url = '/profile/viewpdt/'


class Userpdtview(ListView):
    template_name = 'webapp/userpdt.html'
    model = Product
    context_object_name = 'list'


class Userpdtdetview(DetailView):
    template_name = 'webapp/userproductdetails.html'
    model = Product
...
@csrf_exempt
def success(request):
    return render(request, "webapp/success1.html")
...
class Value(View):
    template_name = 'webapp/orderplaced.html'

    def get(self, request):
        qs = Order.objects.filter(orderedBy=request.user).order_by('-id')[:1]
        context = {
            'list': qs
        }
...
def payment1(request, pk):
    ...
    return render(request, 'webapp/payment1.html')


class Buy1(DetailView):
    model = Order
    template_name = 'webapp/payment1.html'
